...
print("invalid link")
No = int(input("enter a valid integer between 1-20 ")) 
if No not in range(1,21):
print("invalid")
def scraping(url, No):
arry_links = []
Response = requests.get(url).text
links = re.findall('href="(/wiki/.*?)"', Response) 
for link in links:
arry_links.append('https://en.wikipedia.org' + link) 
if n == 0:
...
for link in arry_links:
writer.writerow([link]) 
writer.writerow(["Totalcount:", count]) 
writer.writerow(["uni",lent])
